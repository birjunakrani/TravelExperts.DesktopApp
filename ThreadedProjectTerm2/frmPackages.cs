...
namespace ThreadedProjectTerm2
{
    public partial class frmPackages : Form
    {
        ...
        private void btnAddPkg_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                //call all methods from validator to perform buisness rules checks
                if (Validator.IsPresent(txtPkgName) && (Validator.IsPresent(txtPkgBasePrice) && (Validator.isNonNegative(txtPkgBasePrice, "Base Price")) && (Validator.IsCompare(txtPkgBasePrice,txtPkgAgencyCom) )))
                {
        ...
        private void btnUpdatePkg_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                //checks if the record from the grid is selected and all controls are prepoluated
